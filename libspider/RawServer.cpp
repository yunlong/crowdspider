...
void TRawServer::Run( void )
{
    ...
    if( Init() )
    {
        ...
        while( running )
        {
            ...
            if( rc < 0 )
            {
                ShowMsg("poll failed:error=%i (%s)",errno,strerror(errno));
                break;
            }
...
void TRawServer::Daemonize( void )
{
  /* Ignore BSD terminal stop signals */
    ...
    if( chdir( "/" ) !=  0 )
    {
        ShowMsg( "chdir failed (%s)", strerror(errno) );
    }
    ...
    if( (childpid = fork()) < 0 )
    {
        ShowMsg( "fork failed (%s)", strerror(errno) );
        return;
    }
    else if( childpid > 0 )
    {
    ...
    if( setsid() < 0 )
    {
        ShowMsg( "setsid failed (%s)", strerror(errno) );
        return;
    }
