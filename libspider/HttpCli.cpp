...
THttpCli :: THttpCli(void) : TcpSocket()
{
	...
	IniFile cfg("../build/crowdspider.conf");
...
bool THttpCli :: connectRemoteHost(const string& sUrl)
{
	...
	if(scheme == "http" )
	{
		...
		if(ret == true) 
		{ 
			ShowMsg("Connecting to remote server %s:%d successfully", ipaddr, dport); 
		}
		else 
		{ 
			ShowMsg("Connecting remote server %s:%d failed", ipaddr, dport); 
			return false;
		}
		enableSSL(false);	
	}
...
int THttpCli :: sendHttpRequest( THttpVerb verb, bool useproxy )
{
	...
	if(htRequest != NULL && htRequest->GetHdrCount() > 0)
	{
		if(verb == VERB_GET)
		{
			if(useproxy)
			{
				snprintf(linebuf, MAX_LINE_LEN, "GET %s HTTP/1.1\r\n", reqpath.c_str());
				snprintf(szBuffer, 1024, "GET %s HTTP/1.1\r\n", reqpath.c_str());
		...
		if(verb == VERB_CONNECT)
		{
			...
			{	
				...
				snprintf(linebuf, MAX_LINE_LEN, "CONNECT %s HTTP/1.1\r\n", sites);
				snprintf(szBuffer, 1024, "CONNECT %s HTTP/1.1\r\n", sites);
				pos += 19 + strlen(sites);
			}
			else
			{
...
int THttpCli :: setupHttpSession(const string& sUrl)
{
	...
	if(UseProxy)
	{
		...
		bool status = connectProxyHost( m_proxy );
	    if(status == true) 
		{
	...
	//	htRequest->SetHdr("Connection", "Keep-Alive");	
		...
		if(UseProxy)
		{
			...
			{
				buildProxyAuthHdr(m_proxy.user.c_str(), m_proxy.pass.c_str());
			}
			buildPragmaHdr();
		}
    }
	...
	ShowMsg("%s",getStatusLine().c_str() );
	int nHdr;
	THttpHdr * pHdr;
	nHdr = getHttpRequest()->GetHdrCount();
	for(int i = 0; i < nHdr; i++)
	{
		pHdr = getHttpRequest()->GetHdr( i );
		ShowMsg("%s: %s", pHdr->GetKey().c_str(), pHdr->GetVal().c_str());
	}

	int retcode  = recvHttpResponse();

	ShowMsg("\n%s", getStatusLine().c_str() );
	nHdr = getHttpResponse()->GetHdrCount();
	for(int i = 0; i < nHdr; i++)
	{
		pHdr = getHttpResponse()->GetHdr( i );
		ShowMsg("%s: %s", pHdr->GetKey().c_str(), pHdr->GetVal().c_str());
	}
...
int THttpCli :: fetchHtmlPage(string& sUrl, const string& filename)
{
    ...
    int status = setupHttpSession(sUrl);
